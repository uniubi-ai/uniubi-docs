...
[English](https://github.com/uniubi-ai/uniubi-docs/blob/main/README.md) | **简体中文**
...
> 需要连接真实机器人时，先阅读 [机器人网络接入](https://github.com/uniubi-ai/uniubi-docs/blob/main/docs/core-concepts/device-network.zh-CN.md)，从 App 获取设备 IP，确认登录地址、对外服务端口和通信网卡；仅进行 Mock / Sim2Sim、离线构建或训练时可按需跳过。

### 1. 选择控制模式

| 你要做什么 | 控制模式 | 先读哪篇导读 |
|---|---|---|
| 使用机器人内置动作能力，不单独控制每个关节 | High-level | [High-level：使用机器人内置动作](https://github.com/uniubi-ai/uniubi-docs/blob/main/docs/how-to/high-level-control.zh-CN.md) |
| 自己训练或运行控制策略，直接控制关节位置或扭矩 | Low-level | [Low-level：自定义关节控制策略](https://github.com/uniubi-ai/uniubi-docs/blob/main/docs/how-to/low-level-control.zh-CN.md) |
...
两种方式调用相同的 High-level 能力，实时运动服务始终运行在机器人端。设备 ID 可在 Uniubi App 的“基础信息”页面查看，也可通过 SDK 设备发现获取。完整边界见 [High-level 双部署架构](https://github.com/uniubi-ai/uniubi-docs/blob/main/docs/core-concepts/README.zh-CN.md#high-level-应用的两种部署位置)。
...
- [构建和安装 SDK](https://github.com/uniubi-ai/uniubi-docs/blob/main/docs/BUILD.zh-CN.md)：准备本机或板端的 C++ / Python 开发环境。
- [使用 High-level 控制](https://github.com/uniubi-ai/uniubi-docs/blob/main/docs/how-to/high-level-control.zh-CN.md)：通过外部电脑或机器人“大脑”使用 High-level 接口，读取状态并调用内置动作；目标可为 Mock / Sim2Sim 或真实机器人。
- [使用 ROS 2 Motion Bridge](https://github.com/uniubi-ai/uniubi-docs/blob/main/docs/how-to/ros2-motion-bridge.zh-CN.md)：通过 ROS 2 接入 High-level 能力。
- [使用 Low-level 控制](https://github.com/uniubi-ai/uniubi-docs/blob/main/docs/how-to/low-level-control.zh-CN.md)：先在安全吊架上验证姿态，再到空旷平整地面验证策略行走。
- [训练、导出和回放策略](https://github.com/uniubi-ai/uniubi-docs/blob/main/docs/how-to/train-export-replay.zh-CN.md)：开发并部署自己的 Low-level 策略。
...
[进入核心概念](https://github.com/uniubi-ai/uniubi-docs/blob/main/docs/core-concepts/README.zh-CN.md)
...
[进入操作指南](https://github.com/uniubi-ai/uniubi-docs/blob/main/docs/how-to/README.zh-CN.md)
...
[进入 API 参考](https://github.com/uniubi-ai/uniubi-docs/blob/main/docs/api-reference/README.zh-CN.md)
...
[进入高级主题](https://github.com/uniubi-ai/uniubi-docs/blob/main/docs/advanced/README.zh-CN.md)
...
再开始使用遥控器接管。Low-level 程序要接入遥控器输入时，遥控器也必须处于这个已连接状态；
这是接入遥控器输入的前置条件，不表示退出 Low-level 模式。
...
本仓库中的 UniUbi 原创文档和代码使用 Apache License 2.0。详见 [LICENSE](https://github.com/uniubi-ai/uniubi-docs/blob/main/LICENSE) 和 [NOTICE](https://github.com/uniubi-ai/uniubi-docs/blob/main/NOTICE)。
