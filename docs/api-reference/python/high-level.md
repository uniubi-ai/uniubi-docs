...
| `setObservedEnable` | `set_observed_enable(params=None, timeout_ms=5000)`; requires only `kConnected` and does not perform a control-ownership check. It returns the effective switch dict on success or `None` on failure. Use `start_control()` separately when the application also needs motion control or must switch the target endpoint to master. |
