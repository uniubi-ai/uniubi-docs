...
| `setObservedEnable` | `set_observed_enable(params=None, timeout_ms=5000)`；只要求 `kConnected`，调用本身不检查控制权。成功返回实际开关 dict，失败返回 `None`。如果业务还需要运动控制，或必须将目标端切为 master，再单独调用 `start_control()`。 |
