...
int main() {
    ...
    svc->setNetworkInterface("eth0.100");  // 大脑侧 High-level 必须使用内部通信网卡
...
/// 初始化前指定 SDK 使用的网络接口（如 "eth0"、"wlan0"）
...
4. **不能使用** `lo`（回环）；板载 High-level 使用 `eth0.100`。
...
int main(int argc, char** argv) {
    auto svc = IMotionSdkService::instance();
    svc->setNetworkInterface(argc > 1 ? argv[1] : "eth0");   // 外部主机选实际网卡；板载 High-level 使用 eth0.100
