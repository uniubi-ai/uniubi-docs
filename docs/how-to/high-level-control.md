...
如果你要自己训练或运行控制策略，并直接输出每个关节的位置或扭矩，请转到 [Low-level：自定义关节控制策略](low-level-control.md)。
...
如果使用 SDK，先完成 [SDK 通用准备](sdk-first-use.md)。如果使用 ROS 2，直接进入 [启动并验证 Motion bridge](ros2-motion-bridge.md)。
...
High-level 控制流程和安全边界见 [C++ 高级控制 SDK](../uniubi_high_level_sdk.md) 及 [ROS 2 Motion bridge 导读](ros2-motion-bridge.md)。
