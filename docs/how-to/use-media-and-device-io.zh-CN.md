...
自定义音频文件使用 `add_audio_file()` / `delete_audio_file()`；文件 ID、URL 上传方式和格式限制见 High-level API。语音播放不是麦克风采集接口。

## 自定义音频：通过 URL 上传并播放

播放服务运行在**小脑**上。开发者在大脑或外部主机上通过 HTTP 服务提供音频文件，将小脑可访问的地址作为 `url` 传入；等待音频 ID 入库后再播放。
...
文件也可以放在 PC 的独立目录，通过 `python3 -m http.server 8000` 提供服务；URL 使用 **小脑 能访问的 PC IP**，确认网络路由和防火墙允许访问。请使用小脑可访问的 HTTP URL，不要使用 `localhost`。
...
- HTTP 日志应有来自 小脑 的 `GET` 和 `200`；文件入库后可关闭 HTTP 服务。后续播放使用 ID，无需再次下载。
...
脚本播放一次、音量 50，并释放控制权，不发送运动动作。示例的播放观察上限是 15 秒，长音频应相应调整。若添加后 30 秒仍查不到 ID，检查 HTTP 日志、小脑 网络可达性、格式和存储容量，不要直接继续播放。
