...
|---|---|---|
| 应用运行位置 | PC 或工控机 | 机器人板载计算平台 |
| 常见目标架构 | x86_64 | aarch64 |
| SDK 客户端 | 指定设备 ID（机器人 SN） | 板载单设备客户端，无需设备 ID |
| 通信网卡 | 选择能够到达机器人的实际网卡 | 选择机器人内部通信网卡 |
| High-level 真机 | 支持 | 支持 |
| Low-level 真机 | 不支持 | 支持 |

设备 ID 用来标识 High-level RPC 的目标机器人，IP 地址和通信网卡用来建立网络可达性；两者作用不同。设备 ID 可从 Uniubi App 查看，也可通过 SDK 设备发现获取。

### High-level 与 Low-level 的真机控制路径

High-level 与 Low-level 使用同一套 SDK 家族，但在真实机器人上的部署位置和硬件控制路径不同：
